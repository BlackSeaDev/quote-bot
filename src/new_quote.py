...
import config
from variables import MAIN_MENU_HANDLER, NEW_QUOTE_HANDLER, ADD_Q_OWNER, ADD_NEW_QUOTE, CONFIRMATION_QUOTE_HANDLER
...
def new_quote_handler(update, context):
    ...
    return CONFIRMATION_QUOTE_HANDLER
...
def confirmation_quote_handler(update, context):
    answer = update.message.text
    lang = language(update)
    q_owner = context.chat_data['quote_owner']
    quote_text = context.chat_data['quote_text']
